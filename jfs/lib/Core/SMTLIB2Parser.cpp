...
//                           z3Ctx);
  ...
  // add by zgf : parser version for z3-4.8
  Z3_ast_vector cons_vec = Z3_parse_smtlib2_file(z3Ctx, fileName.str().c_str(),
                                                 0,0,0,0,0,0);
  Z3_ast* cons = new Z3_ast[Z3_ast_vector_size(z3Ctx, cons_vec)];
  for (unsigned i = 0; i < Z3_ast_vector_size(z3Ctx, cons_vec); ++i) {
    cons[i] = Z3_ast_vector_get(z3Ctx, cons_vec, i);
  }
  constraint = Z3ASTHandle(
      Z3_mk_and(z3Ctx, Z3_ast_vector_size(z3Ctx, cons_vec), cons),z3Ctx);
...
//                           z3Ctx);
  ...
  // add by zgf : parser version for z3-4.8
  Z3_ast_vector cons_vec = Z3_parse_smtlib2_string(z3Ctx, str.data(),
                                                 0,0,0,0,0,0);
  Z3_ast* cons = new Z3_ast[Z3_ast_vector_size(z3Ctx, cons_vec)];
  for (unsigned i = 0; i < Z3_ast_vector_size(z3Ctx, cons_vec); ++i) {
    cons[i] = Z3_ast_vector_get(z3Ctx, cons_vec, i);
