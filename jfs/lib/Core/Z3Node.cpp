...
// jfs add by zgf to support Array
bool Z3SortHandle::isArrayTy() const { return getKind() == Z3_ARRAY_SORT; }
...
// jfs add by zgf: check a variable whether model by Array
bool Z3ASTHandle::isArrayVariable() const {
  if (!isApp())
    return false;
  if (isFreeVariable() && getSort().isArrayTy() && !isConstArray())
    return true;
  return false;
}

// jfs add by zgf: check this variable whether is
// const array
bool Z3ASTHandle::isConstArray() const {
  if (!isApp() || !isFreeVariable())
    return false;
  std::string nodeName = toStr();
  if (nodeName.find("const_arr") != std::string::npos)
    return true;
  return false;
}

// jfs add by zgf: check a select statement whether about
// const Array assignment
bool Z3ASTHandle::isConstArraySelect() const {
  if (!isApp() || asApp().getKind() != Z3_OP_SELECT)
    return false;
  return asApp().isConstArraySelect();
}
...
// jfs add by zgf : if this var is 'constArr' it is not use for 'freeVar'
bool Z3AppHandle::isConstArraySelect() const {
  std::string nodeName = getKid(0).toStr();
  Z3AppHandle constNumber = getKid(1).asApp();

  if (nodeName.find("const_arr") != std::string::npos &&
      constNumber.isConstant())
    return true;
  return false;
}
...
//      Z3_apply_result_convert_model(context, node, index, toConvert), context);
  ...
  // add by zgf for z3-4.8
  return Z3ModelHandle(Z3_goal_convert_model(
                           context, getGoal(index), toConvert), context);
}
}
}
