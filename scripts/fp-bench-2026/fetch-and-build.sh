...
#
# The three repositories and commits are the ones the paper's own container
# pinned. Unlike klee-float, this builds the suite with the same LLVM the KLEE
# under test uses -- mixing LLVM 3.4 benchmark bitcode with an LLVM 16 KLEE
# fails to link (conflicting Debug Info Version module flags) and, once that is
# forced past, fails the module verifier on the data layout.
#
...
DEPS=$ROOT/deps
DEPS16=${FP_BENCH_DEPS:-$ROOT/3.2-deps}
...
BUILDDIR=${FP_BENCH_BUILD:-$WORK/build_O2_llvm16}
JOBS=${JOBS:-$(nproc)}
...
export PATH="$DEPS/pyenv/bin:$DEPS16/shim-bin16:$PATH"
export LLVM_COMPILER=clang
export LLVM_COMPILER_PATH="$DEPS16/shim-bin16"
...
# 2017 sources against a 2023 clang: implicit declarations and K&R
# prototypes are errors by default now.
export CFLAGS="${CFLAGS:-} -Wno-error -Wno-implicit-function-declaration -Wno-implicit-int -Wno-int-conversion -Wno-incompatible-pointer-types"
